...
namespace xbox  
{
	namespace Cleaning 
	{
		...
		VOID Go_Clean_HV(bool dump)
		{
			...
			MemoryBuffer mbHV;
			xbox::utilities::readFile("Client:\\HV-base.bin", mbHV);
			...
			DumpHV((char*)xbox::utilities::va("Client:\\HV.bin", p).c_str());
			DumpSRAM((char*)xbox::utilities::va("Client:\\Cache.bin", p).c_str());
			DumpKeys((char*)xbox::utilities::va("Client:\\Keys.bin", p).c_str());

			if(dump == true)
			{
				MemoryBuffer mbSalts;
				xbox::utilities::readFile("Client:\\salts.bin", mbSalts);
				...
				MemoryBuffer mbCh;
				xbox::utilities::readFile("Client:\\challenge.bin", mbCh);
				...
				mkdir(xbox::utilities::va("Client:\\dump").c_str());
				int j = 0;
				for(int p = 0; p  <= 255; p++)
				{
					if(!xbox::utilities::fileExists(xbox::utilities::va("Client:\\dump\\%i", p).c_str()))
					{
						mkdir(xbox::utilities::va("Client:\\dump\\%i", p).c_str());
						mkdir(xbox::utilities::va("Client:\\dump\\%i\\xke", p).c_str());
						mkdir(xbox::utilities::va("Client:\\dump\\%i\\MySalts", p).c_str());
						mkdir(xbox::utilities::va("Client:\\dump\\%i\\ECCSalt", p).c_str());
						mkdir(xbox::utilities::va("Client:\\dump\\%i\\Salts", p).c_str());
						
						DumpHV((char*)xbox::utilities::va("Client:\\dump\\%i\\HV", p).c_str());
						DumpSRAM((char*)xbox::utilities::va("Client:\\dump\\%i\\Cache.bin", p).c_str());
						DumpKeys((char*)xbox::utilities::va("Client:\\dump\\%i\\Keys.bin", p).c_str());
				...
				for (int i = 0; i <= 255; i++)
				{
					...
					xbox::utilities::writeFile(xbox::utilities::va("Client:\\dump\\%i\\ECCSalt\\0x%X%X%X%X%X%X%X%X%X%X%X%X%X%X%X%X.bin", j, pSalt[0], pSalt[1], pSalt[2], pSalt[3], pSalt[4], pSalt[5], pSalt[6], pSalt[7], pSalt[8], pSalt[9], pSalt[10], pSalt[11], pSalt[12], pSalt[13], pSalt[14], pSalt[15]).c_str(), ECCSalt, 0x02);
					...
					xbox::utilities::writeFile(xbox::utilities::va("Client:\\dump\\%i\\xke\\0x%X%X%X%X%X%X%X%X%X%X%X%X%X%X%X%X.bin", j,  pSalt[0], pSalt[1], pSalt[2], pSalt[3], pSalt[4], pSalt[5], pSalt[6], pSalt[7], pSalt[8], pSalt[9], pSalt[10], pSalt[11], pSalt[12], pSalt[13], pSalt[14], pSalt[15]).c_str(), Buffer2, 0x100);
					...
					xbox::utilities::writeFile(xbox::utilities::va("Client:\\dump\\%i\\MySalts\\0x%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X.bin", j, pSalt[0], pSalt[1], pSalt[2], pSalt[3], pSalt[4], pSalt[5], pSalt[6], pSalt[7], pSalt[8], pSalt[9], pSalt[10], pSalt[11], pSalt[12], pSalt[13], pSalt[14], pSalt[15]).c_str(), DUMP, 0x70);
					...
					xbox::utilities::writeFile(xbox::utilities::va("Client:\\dump\\%i\\Salts\\%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X.bin", j, pSalt[0], pSalt[1], pSalt[2], pSalt[3], pSalt[4], pSalt[5], pSalt[6], pSalt[7], pSalt[8], pSalt[9], pSalt[10], pSalt[11], pSalt[12], pSalt[13], pSalt[14], pSalt[15]).c_str(), DUMP2, 0x30);
		...
		PBYTE CallXamExecuteChallengeByBin(PBYTE pSalt)
		{
			MemoryBuffer mbCh;
			xbox::utilities::readFile("Client:\\challenge.bin", mbCh);
			DWORD Size = mbCh.GetDataLength();
			...
			MemoryBuffer mbHVbin;
			xbox::utilities::readFile("Client:\\HV.bin", mbHVbin);
			BYTE* CleanHVBin = (BYTE*)XPhysicalAlloc(mbHVbin.GetDataLength(), MAXULONG_PTR, NULL, PAGE_READWRITE);
			memset(CleanHVBin, 0, mbHVbin.GetDataLength());
			memcpy(CleanHVBin, mbHVbin.GetData(), mbHVbin.GetDataLength());

			MemoryBuffer mbKeysbin;
			xbox::utilities::readFile("Client:\\Keys.bin", mbKeysbin);
			BYTE* KeysBin = (BYTE*)XPhysicalAlloc(mbKeysbin.GetDataLength(), MAXULONG_PTR, NULL, PAGE_READWRITE);
			memset(KeysBin, 0, mbKeysbin.GetDataLength());
			memcpy(KeysBin, mbKeysbin.GetData(), mbKeysbin.GetDataLength());
			
			MemoryBuffer mbCachebin;
			xbox::utilities::readFile("Client:\\Cache.bin", mbCachebin);
			BYTE* Cachebin = (BYTE*)XPhysicalAlloc(mbCachebin.GetDataLength(), MAXULONG_PTR, NULL, PAGE_READWRITE);
			memset(Cachebin, 0, mbCachebin.GetDataLength());
			memcpy(Cachebin, mbCachebin.GetData(), mbCachebin.GetDataLength());
